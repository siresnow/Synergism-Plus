...
const buyGenerator = (i: number, state: boolean, auto: boolean) => {
    if (i === 1 && player.prestigePoints >= 1e12 && player.unlocks.generation === false) {
...
export const autoUpgrades = () => {
    if (player.upgrades[90] > 0.5 && player.shoptoggles.generators === true) {
        ...
        for (let i = 1; i < 6; i++) {
            if (player.prestigePoints >= G['upgradeCosts'][(100 + i)] && player.shoptoggles.generators === true) {
                buyGenerator(i, true, true)
            }
        }
        for (let j = 6; j < 11; j++) {
            if (player.coins >= G['upgradeCosts'][(100 + j)] && player.shoptoggles.generators === true) {
                buyGenerator(j, true, true)
            }
        }
        for (let k = 11; k < 16; k++) {
            if (player.prestigePoints >= G['upgradeCosts'][(100 + k)] && player.shoptoggles.generators === true) {
                buyGenerator(k, true, true)
            }
        }
        for (let l = 16; l < 21; l++) {
            if (player.transcendPoints >= G['upgradeCosts'][(100 + l)] && player.shoptoggles.generators === true) {
    ...
    if (player.upgrades[91] > 0.5) {
        for (let i = 1; i < 21; i++) {
            if (player.coins >= G['upgradeCosts'][i] && player.shoptoggles.coin === true) {
                buyUpgrades('coin', i, true)
            }
        }
        for (let i = 121; i <= 125; i++){
            if (player.coins >= G['upgradeCosts'][i] && player.shoptoggles.coin === true && player.cubeUpgrades[19] > 0) {
                buyUpgrades('coin',i,true)
            }
        }
    }
    if (player.upgrades[92] > 0.5) {
        for (let i = 21; i < 38; i++) {
            if (player.prestigePoints >= G['upgradeCosts'][i] && player.shoptoggles.prestige === true) {
                buyUpgrades('prestige', i, true)
            }
        }
        if (player.prestigePoints >= 1e20 && player.shoptoggles.prestige === true && player.achievements[120] === 1) {
            buyUpgrades('prestige', 38, true)
        }
        if (player.prestigePoints >= 1e40 && player.shoptoggles.prestige === true && player.achievements[127] === 1) {
            buyUpgrades('prestige', 39, true)
        }
        if (player.prestigePoints >= 1e80 && player.shoptoggles.prestige === true && player.achievements[134] === 1) {
    ...
    if (player.upgrades[99] > 0.5) {
        for (let i = 41; i < 61; i++) {
            if (player.transcendPoints >= G['upgradeCosts'][i] && player.shoptoggles.transcend === true) {
    ...
    if (player.cubeUpgrades[8] > 0) {
        for (let i = 61; i <= 80; i++) {
            if (player.reincarnationPoints >= G['upgradeCosts'][i] && player.shoptoggles.reincarnate) {
