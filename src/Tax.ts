...
export const calculatetax = () => {
    ...
    let exponent = player.toggles[35]?2.5:1;
    exponent *= e;
    ...
    let mxe = Math.floor((player.toggles[35]?.5:275) / (Decimal.log(player.toggles[35]?1.05:1.01, 10) * exponent)) - 1
    G['maxexponent'] = mxe;
    ...
    if (a2 >= 1) {
        c = Math.pow(a2, 2) / (player.toggles[35]?1:550)
    }


    compareC = Math.pow(mxe, 2) / (player.toggles[35]?1:550)
    ...
    if (!player.toggles[33]){
        ...
        G['taxdivisor'] = Decimal.pow(base, (c) * (exponent)).times(player.toggles[35]?(G["produceTotal"].plus(1).log10()+1):1)
        G['taxdivisorcheck'] = Decimal.pow(base, (compareC) * (exponent)).times(player.toggles[35]?(G["produceTotal"].plus(1).log10()+1):1)
    }
    else{
